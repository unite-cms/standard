...
class CoreBundleIntegrationTest extends DatabaseAwareTestCase
{
  ...
  public function testCoreBundleIntegration() {
    ...
    $crawler = $this->client->request('GET', $this->container->get('router')->generate('unitedcms_core_domain_view', [
      'organization' => $this->organization->getIdentifier(),
      'domain' => $this->domain->getIdentifier(),
    ]));
    $this->assertEquals(200, $this->client->getResponse()->getStatusCode());
    $this->assertGreaterThan(0, $crawler->filter('.uk-nav-default')->count());
  ...
  public function testCollectionFieldBundleIntegration() {
    ...
    $collection_field->setIdentifier('ctf1')->setType('collection')->setTitle('CTF 1')->setSettings(new FieldableFieldSettings([
      'fields' => [
        [
          'title' => 'Sub Field 1',
          'identifier' => 'sf1',
    ...
    $this->client->request('POST', $this->container->get('router')->generate('unitedcms_core_api', [
      ...
      'CONTENT_TYPE' => 'application/json',
      'HTTP_AUTHENTICATION_FALLBACK' => 'true',
    ], json_encode(['query' => 'query { findCt1 { result { ctf1 { sf1 } } } }']));
    $this->assertEquals(200, $this->client->getResponse()->getStatusCode());
    $response = json_decode($this->client->getResponse()->getContent());
    $this->assertEquals($rand_value, $response->data->findCt1->result[0]->ctf1[0]->sf1);
  }
}